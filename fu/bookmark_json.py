    ...
    def handle(node):
        ...
        if not "children" in node:
            ...
            if not re.match("^\w+://", node["url"]):
                ...
                exit(-1)
            return f"<DT><A HREF=\"{node['url']}\">{label}</A>\n"
        s = "".join(map(handle, node["children"]))
        return f"<DT><H3>{label}</H3>\n<DL>\n{s}\n</DL>\n"
...
def bookmark_file2json(bookmark_file: str):
    if not os.path.exists(bookmark_file):
        raise Exception(f"{bookmark_file}不存在")
    return bookmark2json(open(bookmark_file, encoding="utf8").readlines())


def bookmark2json(lines: List[str]):
    i = 0
    ...
    while i < len(lines):
        ...
        if line.startswith("<DL>"):
            ...
            it = {"label": ele.text()}
            if "HREF=" in line:
                url = re.search('HREF\s*=\s*"(.+?)"', line).group(1)
                it["url"] = url
            stack[-1].append(it)
...
def main():
    ...
    if filename.endswith(".json"):
        ...
        node = bookmark_file2json(filename)
